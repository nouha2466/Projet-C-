...
void dessinerDroite(const std::string& filename, int largeur, int hauteur, double m, double b) {
    std::ofstream fichierImage(filename); // Crée un fichier pour l'image PPM
    if (!fichierImage) {
    ...
    for (int i = 0; i < hauteur; ++i) {
        for (int j = 0; j < largeur; ++j) {
            fichierImage << image[i][j].r << " " << image[i][j].g << " " << image[i][j].b << " ";
        }
        fichierImage << "\n";
    }

    fichierImage.close(); // Fermer le fichier
}
...
int main() {
    ...
    // Exemple d'utilisation de la fonction dessinerDroite
    dessinerDroite("output.ppm", 800, 600, 1.0, 100.0);

    // Exemple d'utilisation de la fonction houghTransformNaive
    PPMImage image; // Initialisez votre image ici
